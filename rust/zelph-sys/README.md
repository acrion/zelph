...
This crate declares `links = "zelph"` and publishes `DEP_ZELPH_LIB_DIR`, which cargo hands to its **direct** dependents only. A binary requiring the rpath therefore names this crate itself, not merely `zelph`; see [the documentation](https://acrion.github.io/zelph/rust/).
