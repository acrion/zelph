...
std::shared_ptr<DataManager> DataManager::create(network::Zelph* n, const std::filesystem::path& input_path)
{
    ...
    if (!source_path.empty())
    {
        ...
        if (ext == ".json" || ext == ".bz2")
        {
            return std::make_shared<wikidata::Wikidata>(n, input_path);
        }
    }
...
void GenericDataManager::load()
{
    ...
    _n->diagnostic("Loading network from generic file " + _input_path.string() + "...", true);
    _n->load_from_file(_input_path.string());
    _n->diagnostic("Network loaded.", true);
}
