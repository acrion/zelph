...
{
    return _pImpl->_stop == std::chrono::time_point<std::chrono::high_resolution_clock>::min();
}

void StopWatch::start()
{
    _pImpl->_start = std::chrono::high_resolution_clock::now();
    _pImpl->_stop  = std::chrono::time_point<std::chrono::high_resolution_clock>::min();
}
...
{
    auto currentTime = is_running() ? std::chrono::high_resolution_clock::now() : _pImpl->_stop;
    return std::chrono::duration_cast<std::chrono::milliseconds>(currentTime - _pImpl->_start).count();
}
