...
Variables in the zelph syntax are single uppercase letters or words that are prefixed with an underscore `_`.
Note that the scope of variables always is a single rule.
...
Reasoning complete. Total unification matches processed: 655. Total contradictions found: 0.
Reasoning summary: 655 matches processed, 0 contradictions found.
Parallel unifications activated for 0 distinct fixed relations.
Reasoning complete in 0h0m0.012s – 655 matches processed, 0 contradictions found.
Ready.
zelph>
```

The results demonstrate zelph’s powerful inference capabilities. It not only answers the specific query about who is an ancestor of pius: After entering `.run`, it also derives numerous other facts based on the rules and base facts provided in the script.
