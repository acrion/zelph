...
Each section is divided into multiple chunks. For example, the pruned Wikidata 2026 file contains 75 left chunks, 75 right chunks, 21 nameOfNode chunks, and 21 nodeOfName chunks.
...
The resulting JSON file lists the byte offset and length of each chunk within the `.bin` file. This information is needed for creating manifest files (see [Manifest-Based Loading](#manifest-based-loading) below) and is useful for understanding the internal layout of a `.bin` file.
...
For advanced use cases — especially when hosting `.bin` files remotely or as pre-split shard files — zelph supports loading via a **manifest file**. A manifest is a JSON file that describes the chunk layout of a `.bin` file: where each chunk is located, how large it is, and optionally where to fetch it from.
...
```json
{
  ...
  "left": [
    {"chunkIndex": 0, "offset": 31, "length": 232195040, "which": "left"},
    ...
  ],
  "right": [...],
  "nameOfNode": [...],
...
To turn this into a manifest, wrap the sections and add a `source` object pointing to the original `.bin` file:
...
The key differences from the raw index are:

- The chunk arrays are nested under a `sections` object.
- A `source` object provides `binPath` (path to the `.bin` file) and `headerLengthBytes` (byte length of the header, taken from the `header.length` field in the index).
- Each chunk entry needs at minimum `chunkIndex`, `offset` (or `sourceOffset`), and `length`.

For sharded layouts, each chunk entry can additionally contain an `objectPath` field pointing to a local file path or a remote URL (`hf://` or `https://`). When `objectPath` is present, zelph reads the chunk from that file instead of seeking into the source `.bin`.
...
When chunks reference remote URLs, zelph fetches them automatically using `curl` and caches them under a temporary directory. If `shard-root` is set, zelph first looks for matching files there before attempting a remote download.

### Planned: Pre-Sharded Files on Hugging Face

We are working on publishing pre-sharded versions of the available `.bin` files along with ready-to-use manifest files on [Hugging Face](https://huggingface.co/datasets/acrion/zelph). This will allow partial loading of Wikidata networks directly from the cloud without downloading entire multi-gigabyte files first.
...
The partial loading infrastructure — `.load-partial`, `.stat-file`, `.index-file`, chunk selection, manifest-based loading, and remote shard support — was contributed by [chboishabba](https://github.com/chboishabba). Many thanks for this substantial contribution!
